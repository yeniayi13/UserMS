...
namespace UserMs.Test.Infrastructure.Repositories
{
    public class RoleRepositoryTests
    {
        ...
        public RoleRepositoryTests()
        {
            ...
            _repository = new RoleRepository(_dbContextMock.Object, _mongoContextMock.Object, _mapperMock.Object);
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new RoleRepository(null, _mongoContextMock.Object, _mapperMock.Object));
        }

        [Fact]
        public void Constructor_ShouldThrowException_WhenMongoContextIsNull()
        {
            Assert.Throws<ArgumentNullException>(() => new RoleRepository(_dbContextMock.Object, null, _mapperMock.Object));
        }
        ...
        public void Constructor_ShouldThrowException_WhenMongoDatabaseIsNull()
        {
            ...
            Assert.Throws<ArgumentNullException>(() => new RoleRepository(_dbContextMock.Object, _mongoContextMock.Object, _mapperMock.Object));
        }

        [Fact]
        public void Constructor_ShouldThrowException_WhenMapperIsNull()
        {
            Assert.Throws<ArgumentNullException>(() => new RoleRepository(_dbContextMock.Object, _mongoContextMock.Object, null));
        }
        ...
        public async Task GetRolesByIdQuery_ShouldReturnNull_WhenRoleNotFound()
        {
            ...
            mockCursor.SetupSequence(c => c.MoveNextAsync(It.IsAny<CancellationToken>()))
                .ReturnsAsync(false);
            mockCursor.SetupGet(c => c.Current).Returns(new List<GetRoleDto>()); // 🔹 Asegurar que `Current` no es `null`
        ...
        [Fact]
        public async Task DeleteRolesAsync_ShouldDeleteRoleSuccessfully()
        {
            var roleId =  RoleId.Create(Guid.NewGuid());
            var roleName = RoleName.Create("Admin");
            var existingRole = new Roles { RoleId = roleId.Value, RoleName = roleName };

            _dbContextMock.Setup(db => db.Roles.FindAsync(roleId))
                .ReturnsAsync(existingRole);

            _dbContextMock.Setup(db => db.SaveEfContextChanges(It.IsAny<string>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(true);

            var result = await _repository.DeleteRolesAsync(roleId);

            Assert.NotNull(result);
            Assert.Equal(roleId,result.RoleId);

            _dbContextMock.Verify(db => db.Roles.FindAsync(roleId), Times.Once);
